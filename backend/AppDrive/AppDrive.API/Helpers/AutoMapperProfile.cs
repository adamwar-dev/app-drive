using AppDrive.API.Entities;
using AppDrive.API.Models.Accounts;
using AppDrive.API.Models.Categories;
using AppDrive.API.Models.Folders;
using AppDrive.API.Models.Images;
using AutoMapper;

namespace AppDrive.API.Helpers
{
    public class AutoMapperProfile: Profile
    {
        public AutoMapperProfile()
        {
            CreateMap<Account, AccountResponse>();

            CreateMap<Account, AuthenticateResponse>();

            CreateMap<ExternalAuthRequest, Account>();

            CreateMap<Account, GetAccountInfoResponse>();

            CreateMap<RegisterRequest, Account>();

            CreateMap<CreateRequest, Account>();

            CreateMap<UpdateRequest, Account>()
                .ForAllMembers(x => x.Condition(
                    (src, dest, prop) =>
                    {
                        // ignore null & empty string properties
                        if (prop == null) return false;
                        if (prop.GetType() == typeof(string) && string.IsNullOrEmpty((string)prop)) return false;

                        // ignore null role
                        if (x.DestinationMember.Name == "Role" && src.Role == null) return false;

                        return true;
                    }
                ));

            CreateMap<AddCategoryRequest, Category>();

            CreateMap<Category, CategoryResponse>();

            CreateMap<AddFolderRequest, Folder>();

            CreateMap<Folder, GetFolderResponse>();

            CreateMap<Image, GetImageResponse>();

            CreateMap<Folder, GetMainFoldersResponse>();

            CreateMap<Folder, GetFolderListResponse>();

            CreateMap<ImageCategory, ImageCategoriesDto>();
        }
    }
}
